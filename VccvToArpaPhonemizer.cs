...
namespace VccvToArpa {
    ...
    /// EN ARPA LITEとは別のVCCV変換専用プラグイン。
    /// </summary>
    public sealed class VccvToArpaPhonemizer : SyllableBasedPhonemizer {
        protected override string YamlFileName => "vccv-to-arpa.yaml";
        protected override string YamlVersion => "1.3";
        protected override byte[] YamlTemplate => LoadTemplate();

        // Keep word conversion independent from SyllableBasedPhonemizer's asynchronous
        // dictionary bootstrap. Some external-plugin environments can discover the DLL
        // while leaving that bootstrap incomplete, which makes every word resolve empty.
        private IG2p activeG2p = new ArpabetPlusG2p();
        private bool vccvInputMode;
        ...
        public override void SetUp(Note[][] notes, UProject project, UTrack track) {
            ...
            vccvInputMode = (notes ?? Array.Empty<Note[]>())
                .Where(group => group != null && group.Length > 0)
                .Select(group => group[0])
                .Where(note => string.IsNullOrEmpty(note.phoneticHint))
                .Any(note => VccvEnglishAliasParser.TryParse(note.lyric, out var parsed)
                    && parsed.StrongSignature);
        }
        
        protected override string[] GetVowels() => vowels;
        protected override string[] GetConsonants() => consonants;
        protected override double GetSyllableTimingWeight(string vowel) =>
            vowel == "ax" ? 0.62 : 1.0;
        protected override string GetDictionaryName() => null;
        private new List<string> consExceptions = new List<string>();

        /// <summary>
        /// Uses ah as the primary central vowel. The singer-aware alias stage
        /// falls back from ah to aa only when the requested ah alias is absent.
        /// </summary>
        public static string ReduceLiteVowel(string phoneme) => phoneme switch {
            "ax" => "ah",
            _ => phoneme,
        };

        private static string ReduceLiteAlias(string alias) {
            if (string.IsNullOrEmpty(alias)) {
                return alias;
            }
            return Regex.Replace(alias, @"(?<![A-Za-z])ax(?![A-Za-z])", "ah");
        }

        protected override string NormalizeFinalAlias(string alias) => ReduceLiteAlias(alias);

        protected override string GetAliasFallback(string alias, int tone) {
            var aaAlias = Regex.Replace(
                alias ?? string.Empty,
                @"(?<![A-Za-z])ah(?![A-Za-z])",
                "aa");
            return aaAlias != alias && HasOto(aaAlias, tone) ? aaAlias : null;
        }

        protected override string ResolveAliasForSinger(
                string alias, int tone, string color, string alt, USinger singer) {
            if (string.IsNullOrEmpty(alias)
                    || !Regex.IsMatch(alias, @"(?<![A-Za-z])ah(?![A-Za-z])")) {
                return alias;
            }
            bool HasExactAlias(string candidate) {
                return (!string.IsNullOrEmpty(alt)
                        && singer.TryGetMappedOto(candidate + alt, tone, color, out _))
                    || singer.TryGetMappedOto(candidate, tone, color, out _);
            }
            if (HasExactAlias(alias)) {
                return alias;
            }
            var aaAlias = Regex.Replace(
                alias,
                @"(?<![A-Za-z])ah(?![A-Za-z])",
                "aa");
            return aaAlias != alias && HasExactAlias(aaAlias) ? aaAlias : alias;
        }

        /// <summary>
        /// Removes Synthesizer V's leading glottal marker while preserving
        /// apostrophes inside contractions such as "I've" and "can't".
        /// The Lite reclist has no q/cl aliases, so the marker is accepted but
        /// intentionally omitted instead of emitting a missing alias.
        /// </summary>
        public static string NormalizeSynthVLyric(string lyric) {
            var normalized = (lyric ?? string.Empty)
                .Trim()
                .Replace('\u2019', '\'');
        ...
        /// <summary>
        /// Parses USTX lyrics produced from Synthesizer V phoneme overrides.
        /// A leading dot means the remainder is an ARPAbet-like phoneme list.
        /// Unsupported cl is omitted and sil/pau becomes an OpenUtau rest tail.
        /// </summary>
        public static string[] ParseSynthVPhonemeLyric(string lyric) {
            var normalized = (lyric ?? string.Empty).Trim();
            if (!normalized.StartsWith(".", StringComparison.Ordinal)) {
                return Array.Empty<string>();
            }
            return NormalizeSynthVPhonemes(normalized.Substring(1).Split(
                new[] { ' ', '\t' }, StringSplitOptions.RemoveEmptyEntries));
        }

        public static string[] NormalizeSynthVPhonemes(IEnumerable<string> phonemes) {
            var result = new List<string>();
            foreach (var raw in phonemes ?? Array.Empty<string>()) {
                var token = raw.Trim();
                if (token == "R") {
                    result.Add("R");
                    continue;
                }
                token = Regex.Replace(token.ToLowerInvariant(), "[012]$", string.Empty);
                switch (token) {
                    case "":
                    case "cl":
                        // The target Lite bank has no closure alias. Keeping cl
                        // would turn a valid SVP note into a missing phoneme.
                        continue;
                    case "sil":
                    case "pau":
                        result.Add("R");
                        continue;
                    default:
                        result.Add(ReduceLiteVowel(token));
                        continue;
                }
            }
        ...
        /// <summary>
        /// Converts one Cz-style English VCCV alias into its ARPAbet sequence.
        /// This public helper is also used by the regression tests and can be
        /// used to audit imported UST lyrics without loading a singer.
        /// </summary>
        public static string[] ParseVccvAliasToArpabet(string lyric) =>
            VccvEnglishAliasParser.TryParse(lyric, out var parsed)
                ? parsed.Phonemes
                : Array.Empty<string>();

        protected override bool TryGetSelfContainedAliases(
                Note note, out List<string> aliases) {
            aliases = new List<string>();
            if (!string.IsNullOrEmpty(note.phoneticHint)
                    || !VccvEnglishAliasParser.TryParse(note.lyric, out var parsed)) {
                return false;
            }

            // A strong Cz/VCCV marker enables the whole phrase. Compact
            // aliases without markers (for example "hu") are accepted when
            // the phrase has already been identified as VCCV, or when the
            // normal English dictionary has no word by that spelling.
            if (!parsed.StrongSignature && !vccvInputMode) {
                var normalized = NormalizeSynthVLyric(note.lyric).ToLowerInvariant();
                var dictionaryResult = activeG2p.Query(normalized);
                if (dictionaryResult != null && dictionaryResult.Length > 0) {
                    return false;
                }
            }

            aliases = BuildVccvArpasingAliases(parsed, note.tone);
            return aliases.Count > 0;
        }

        private List<string> BuildVccvArpasingAliases(
                VccvEnglishAliasParser.ParsedAlias parsed, int tone) {
            var phonemes = parsed.Phonemes
                .Select(ReduceLiteVowel)
                .ToArray();
            ...
            void AddBoundary(string alias) {
                if (aliases.Count == 0 || aliases[^1] != alias) {
                    aliases.Add(alias);
                }
            }

            void AddTransition(string left, string right) {
                var direct = $"{left} {right}";
                if (HasResolvableVccvAlias(direct, tone)) {
                    AddBoundary(direct);
                    return;
                }

                // Preserve ARPA+'s missing VC/CC strategy for pre-phonemized
                // VCCV notes as well: close the left symbol and restart the
                // right one. This also supplies a usable fallback for a sparse
                // bank that lacks a particular VV or CV transition.
                var leftEnd = $"{left} -";
                var rightStart = $"- {right}";
                var addedFallback = false;
                if (HasResolvableVccvAlias(leftEnd, tone)) {
                    AddBoundary(leftEnd);
                    addedFallback = true;
                }
                if (HasResolvableVccvAlias(rightStart, tone)) {
                    AddBoundary(rightStart);
                    addedFallback = true;
                }
                if (!addedFallback) {
                    AddBoundary(direct);
            ...
            if (phonemes.Length == 1) {
                if (parsed.HasStart) {
                    AddBoundary($"- {phonemes[0]}");
                }
                if (parsed.HasEnd) {
                    AddBoundary($"{phonemes[0]} -");
                }
                if (!parsed.HasStart && !parsed.HasEnd) {
                    var plain = phonemes[0];
                    AddBoundary(HasResolvableVccvAlias(plain, tone)
                        ? plain
                        : $"- {plain}");
                }
                return aliases;
            ...
            for (var index = 0; index + 1 < phonemes.Length; index++) {
                AddTransition(phonemes[index], phonemes[index + 1]);
            }
            if (parsed.HasEnd) {
                AddBoundary($"{phonemes[^1]} -");
            }
            return aliases;
        }

        private bool HasResolvableVccvAlias(string alias, int tone) {
            var normalized = ReduceLiteAlias(alias);
            if (HasOto(normalized, tone)) {
                return true;
            }
            var fallback = GetAliasFallback(normalized, tone);
            if (!string.IsNullOrEmpty(fallback) && HasOto(fallback, tone)) {
                return true;
            }
            var validated = ReduceLiteAlias(ValidateAlias(normalized));
            if (HasOto(validated, tone)) {
                return true;
            }
            fallback = GetAliasFallback(validated, tone);
            return !string.IsNullOrEmpty(fallback) && HasOto(fallback, tone);
        }

        protected override bool IsBoundaryNote(Note note) {
            var lyric = (note.lyric ?? string.Empty).Trim();
            return lyric.Equals("br", StringComparison.OrdinalIgnoreCase)
                || lyric.Equals(".br", StringComparison.OrdinalIgnoreCase);
        }
        ...
        protected override Result ProcessBoundaryNote(Note note) {
            ...
            // normal alias fallback turn a missing breath into raw text.
            if (!string.IsNullOrEmpty(alt)
                    && singer.TryGetMappedOto("br" + alt, tone, color, out var alternateOto)) {
                return new Result {
                    phonemes = new[] { new Phoneme { phoneme = alternateOto.Alias, position = 0 } },
                };
            }
            if (singer.TryGetMappedOto("br", tone, color, out var oto)) {
                return new Result {
                    phonemes = new[] { new Phoneme { phoneme = oto.Alias, position = 0 } },
                };
            }
            return new Result { phonemes = Array.Empty<Phoneme>() };
        }

        protected override bool TryGetStandaloneConsonants(
                Note note, out string[] standaloneConsonants) {
            standaloneConsonants = ParseSynthVPhonemeLyric(note.lyric);
            return standaloneConsonants.Length > 0
                && standaloneConsonants.All(symbol => consonants.Contains(symbol));
        }

        protected override List<string> ProcessStandaloneConsonants(
                Ending? previousEnding,
                string[] standaloneConsonants,
                int tone,
                bool release) {
            var consonantChain = new List<string>();
            if (previousEnding.HasValue) {
                consonantChain.AddRange(previousEnding.Value.cc);
            }
            consonantChain.AddRange(standaloneConsonants);
            var cc = CollapseAdjacentConsonants(consonantChain);
            if (cc.Length == 0) {
                return new List<string>();
            }

            var phonemes = new List<string>();
            var previousVowel = previousEnding?.prevV ?? string.Empty;
            if (!string.IsNullOrEmpty(previousVowel)) {
                var replacedPreviousVowel = ReplacePhoneme(previousVowel, tone);
                var firstConsonant = ReplacePhoneme(cc[0], tone);
                var vc = $"{replacedPreviousVowel} {firstConsonant}";
                if (!TryAddPhoneme(phonemes, tone, vc, ValidateAlias(vc))) {
                    var vowelEnd = AliasFormat(
                        replacedPreviousVowel, "ending", tone, "", "-");
                    var consonantStart = AliasFormat(
                        firstConsonant, "cc_start", tone, "");
                    TryAddPhoneme(phonemes, tone, vowelEnd, ValidateAlias(vowelEnd));
                    TryAddPhoneme(
                        phonemes, tone, consonantStart, ValidateAlias(consonantStart), firstConsonant);
                }
            } else {
                var start = AliasFormat(ReplacePhoneme(cc[0], tone), "cc_start", tone, "");
                TryAddPhoneme(phonemes, tone, start, ValidateAlias(start), cc[0]);
            }

            for (var i = 0; i + 1 < cc.Length; i++) {
                var left = ReplacePhoneme(cc[i], tone);
                var right = ReplacePhoneme(cc[i + 1], tone);
                var direct = $"{left} {right}";
                if (!TryAddPhoneme(phonemes, tone, direct, ValidateAlias(direct))) {
                    var leftEnd = AliasFormat(left, "cc_endB", tone, "");
                    var rightStart = AliasFormat(right, "cc_inB", tone, "");
                    TryAddPhoneme(phonemes, tone, leftEnd, ValidateAlias(leftEnd), left);
                    TryAddPhoneme(phonemes, tone, rightStart, ValidateAlias(rightStart), right);
                }
            }

            if (release) {
                var last = ReplacePhoneme(cc[^1], tone);
                var ending = AliasFormat(last, "ending", tone, "", "-");
                TryAddPhoneme(phonemes, tone, ending, ValidateAlias(ending), last);
            }
            return phonemes.Select(ReduceLiteAlias).ToList();
        }

        protected override List<string> SuppressCarriedConsonantReattack(
                List<string> phonemes,
                string[] carriedConsonants,
                int tone) {
            if (phonemes == null || phonemes.Count == 0 || carriedConsonants.Length == 0) {
                return phonemes ?? new List<string>();
            }
            var candidates = new HashSet<string>(StringComparer.Ordinal);
            foreach (var raw in carriedConsonants) {
                var consonant = ReplacePhoneme(raw, tone);
                foreach (var format in new[] { "cc_start", "cc_inB" }) {
                    var alias = AliasFormat(consonant, format, tone, "");
                    candidates.Add(alias);
                    candidates.Add(ValidateAlias(alias));
                }
                candidates.Add($"- {consonant}");
            }
            return phonemes.Where(alias => !candidates.Contains(alias)).ToList();
        }
        ...
        protected override string[] GetSymbols(Note note) {
            ...
            var normalizedLyric = NormalizeSynthVLyric(note.lyric);

            if (tails.Contains(note.lyric)) {
                original = new[] { note.lyric };
            } else if (phoneticHint) {
                original = NormalizeSynthVPhonemes(note.phoneticHint
                    .TrimStart('.')
                    .Split(' ', StringSplitOptions.RemoveEmptyEntries));
            } else if (note.lyric.TrimStart().StartsWith(".", StringComparison.Ordinal)) {
                original = ParseSynthVPhonemeLyric(note.lyric);
            } else if (string.IsNullOrEmpty(normalizedLyric)) {
                // An apostrophe-only SynthV note represents a glottal closure.
                // Treat it as silence because the Lite bank has no q/cl sample.
                original = new[] { "R" };
            } else {
                var symbols = new List<string>();
                foreach (var subword in normalizedLyric.ToLowerInvariant()
                        .Split(new[] { ' ', '_' }, StringSplitOptions.RemoveEmptyEntries)) {
                    string[] result;
                    if (note.lyric.TrimStart().StartsWith("'", StringComparison.Ordinal)
                            && subword == "n") {
                        // SynthV commonly uses 'n for the reduced "and" /n/.
                        result = new[] { "n" };
                    } else {
                        result = activeG2p.Query(subword);
                        if ((result == null || result.Length == 0) && subword.Contains('\'')) {
                            result = activeG2p.Query(subword.Replace("'", string.Empty));
                        }
                    }
                    if (result == null || result.Length == 0) {
                        Log.Warning($"Subword '{subword}' from word '{note.lyric}' can't be found in the ARPA Lite dictionary");
                        return null;
                    }
                    symbols.AddRange(result);
                }
                // Keep dictionary ax until the base alignment stage so it can
                // act as an unstressed-syllable timing hint. It is converted
                // to ah by the normal replacement stage before alias output.
                original = symbols.ToArray();
            }

            if (original == null) {
                return null;
            }
            List<string> modified = original.ToList();
            // Singer/YAML replacements are applied once by the base class,
            // after it has read the raw vowel symbols for timing weights.
            List<string> finalPhonemes = modified;
        ...
        public override void SetSinger(USinger singer) {
            ...
            try {
                activeG2p = LoadBaseDictionary();
            } catch (Exception e) {
                Log.Error(e, "Failed to initialize the ARPA Lite dictionary. Falling back to built-in ARPAbet+");
                activeG2p = new ArpabetPlusG2p();
            }
        ...
        // Keep ah as the primary central vowel. Alias-level fallback handles
        // ah -> aa only when the singer lacks the requested ah alias.
        private string ReplacePhoneme(string phoneme, int tone) {
            phoneme = ReduceLiteVowel(phoneme);
            if (phoneme == "ah") {
                return phoneme;
            }
            ...
            if (dictionaryReplacements.TryGetValue(phoneme, out var replaced)) {
                return ReduceLiteVowel(replaced);
            }
            return phoneme;
        }
